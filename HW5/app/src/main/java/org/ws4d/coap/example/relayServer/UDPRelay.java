...
public class UDPRelay {
	...
	public void run(InetSocketAddress serverAddr) {
		...
		try {
			...
			Log.v("******", "Start UDP Relay on server port " + SERVER_PORT + " and client port " + CLIENT_PORT);
		} catch (IOException e) {
			//logger.error("Initialization failed, shutting down: " + e.getLocalizedMessage());
			Log.v("******", "Initialization failed, shutting down: " + e.getLocalizedMessage());
			System.exit(-1);
		}
		...
		while (true) {
			...
			try {
				...
				Log.v("******", "Read failed: " + e.getLocalizedMessage());
			}
			...
			if (tempClientAddr != null) {
				...
				try {
					...
					Log.v("******", "Forwarded message: " + "client (" + this.clientAddr.getHostName() + ":"
							+ this.clientAddr.getPort() + ") -> " + "server (" + serverAddr.getHostName() + ":"
							+ serverAddr.getPort() + ") " + "[" + this.clientBuffer.limit() + " bytes]");
				} catch (IOException e) {
					//logger.warn("Send failed: " + e.getLocalizedMessage());
					Log.v("******", "Send failed: " + e.getLocalizedMessage());
				}
			}
			...
			if (serverLen > 0) {
				try {
					...
					Log.v("******", "Forwarded Message: " + "server (" + serverAddr.getHostName() + ":"
							+ serverAddr.getPort() + ") -> " + "client (" + this.clientAddr.getHostName() + ":"
							+ this.clientAddr.getPort() + ") " + "[" + this.serverBuffer.limit() + " bytes]");
				} catch (IOException e) {
					//logger.warn("Send failed: " + e.getLocalizedMessage());
			...
			try {
				...
				Log.v("******", "Select failed: " + e.getLocalizedMessage());
			}
		}
	}
}
