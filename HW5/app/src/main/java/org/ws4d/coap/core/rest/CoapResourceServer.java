...
public class CoapResourceServer implements ResourceServer {
	...
	public boolean createResource(CoapResource resource) {
		if (null != resource && !this.resources.containsKey(resource.getPath())) {
			...
			Log.v("******", "created ressource: " + resource.getPath());
			this.coreResource.changed();
			return true;
		}
		return false;
	...
	public boolean updateResource(CoapResource resource, CoapRequest request) {
		if (null != resource && this.resources.containsKey(resource.getPath())) {
			...
			Log.v("******", "updated ressource: " + resource.getPath());
			return true;
		}
		return false;
	}
	...
	public void start(int serverport) throws Exception {
		...
		Log.v("$$$$$$$$$$", "Server started at port: " + this.port);
		Log.v("$$$$$$$$$$", "Number of resources: " + resources.size());
		if(resources.size() > 0){
			for (String key : resources.keySet()){
				Log.v(">>> ", "KEY: " + key );
				Log.v(">>> ", "VALUE: " + resources.get(key) );
			}
		}
	}
	...
	public URI getHostUri() {
		...
		try {
			...
			Log.v("******", "getHostUri() could not create valid URI from local IP address\n" + e);
		}
		return hostUri;
	}
	...
	public void resourceChanged(CoapResource resource) {
		...
		Log.v("******", "Resource changed: "+ resource.getPath());
	}
	...
	public void onRequest(CoapServerChannel channel, CoapRequest request) {
		...
		Log.v("<<<SERVER>>>", "Incoming request: Channel: " + channel + "; Request: " + request);

		CoapResponse response = null;
		CoapRequestCode requestCode = request.getRequestCode();
		Log.v("<<<SERVER>>>", "request code: " + requestCode);
		String targetPath = request.getUriPath();
		Log.v("<<<SERVER>>>", "target path: " + targetPath);
		int eTagMatch = -1;
	...
	public void onSeparateResponseFailed(CoapServerChannel channel) {
		...
		Log.v("******", "Separate response failed.");
	}
	...
	private static String getLocalIpAddress() {
		try {
			...
			Log.v("******", "Can't obtain network Interface\n" + ex);
		}
		return null;
	}
